...
import { Stethoscope, Plus, Trash2, CheckCircle2, AlertCircle, ShoppingBag, Tag, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Dispense items to patients. The system automatically selects non-expired batches in order of earliest expiry & purchase date (FIFO)</p>
        </div>
        <span className="px-3 py-1 rounded-full bg-brand-orange/20 text-brand-orange border border-brand-orange/30 text-xs font-bold flex items-center gap-1.5">
      ...
      {fifoResult && fifoResult.length > 0 && (
        <div className="p-5 rounded-2xl bg-slate-900 border border-brand-orange/40 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400">Earliest Expiry Sold First</span>
          </div>
            ...
            {fifoResult.map((fb, idx) => (
              <div key={idx} className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-1">
                <div className="flex justify-between font-bold text-slate-200">
                  ...
                  <span className="text-emerald-400">{fb.qty} units</span>
                </div>
      ...
      <form onSubmit={handleSubmit} className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Clinic Outlet *</label>
            <select
              required
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Patient / Customer Name *</label>
            <input
              type="text"
              required
              value={customerName}
        ...
        <div className="pt-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Dispensing Items Cart</h3>
            <button
              type="button"
              onClick={addCartLine}
              className="px-3 py-1.5 rounded-lg bg-brand-orange/20 text-brand-orange border border-brand-orange/40 text-xs font-semibold hover:bg-brand-orange/30 transition-all flex items-center gap-1"
            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-slate-800 gap-4">
          <div className="flex items-center space-x-6">
            <div>
              <p className="text-[10px] text-slate-400">Discount Amount ($):</p>
              <input
                type="number"
                step="0.01"
                min="0"
                ...
                className="w-24 bg-slate-900 border border-slate-800 rounded-lg px-2 py-1 text-xs text-slate-100"
              />
            </div>
            <div>
