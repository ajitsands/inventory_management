...
export default function Sidebar({ activeTab, setActiveTab }) {
  ...
  return (
    <aside className="w-64 bg-slate-900/90 border-r border-slate-800/80 flex flex-col justify-between p-4 min-h-[calc(100vh-4rem)]">
      <div className="space-y-1">
        <div className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 font-heading">
          Navigation Menu
        </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all ${
                isActive
                  ? 'bg-gradient-to-r from-[#1C8DCD] to-[#146ca1] text-white shadow-lg glow-blue font-bold'
      ...
      <div className="pt-4 border-t border-slate-800/60 text-center">
        ...
        <p className="text-[10px] text-slate-400 mt-1">AES-256 Encrypted Route Security</p>
      </div>
    </aside>
  );
}
