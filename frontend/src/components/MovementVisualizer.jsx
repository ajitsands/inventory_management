...
import { Truck, Warehouse, GitBranch, Stethoscope, UserCheck, ArrowRight, ShieldCheck, Tag, Receipt, AlertCircle } from 'lucide-react';
...
export default function MovementVisualizer() {
  const steps = [
    ...
    },
    {
      ...
      icon: Warehouse,
      color: "border-brand-blue/50 bg-brand-blue/10 text-brand-blue",
    ...
    },
    {
      ...
      icon: UserCheck,
      color: "border-brand-orange/50 bg-brand-orange/10 text-brand-orange",
      badge: "FIFO Auto Sale"
    }
  ];

  return (
    <div className="glass-panel p-6 rounded-2xl border border-slate-800 shadow-xl mb-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Strict FIFO stock movement pipeline with audit logging</p>
        </div>
        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-emerald-950 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5">
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={idx} className="relative group">
              <div className={`p-4 rounded-xl border ${step.color} glass-panel-hover h-full flex flex-col justify-between`}>
                <div>
                  ...
                  <h3 className="text-xs font-bold text-slate-100 font-heading">{step.title}</h3>
                  <p className="text-[11px] font-semibold text-slate-400 mb-1">{step.subtitle}</p>
              ...
              {idx < steps.length - 1 && (
                <div className="hidden md:flex absolute -right-3 top-1/2 -translate-y-1/2 z-10 bg-slate-900 border border-slate-800 text-slate-400 rounded-full p-1 shadow-md">
